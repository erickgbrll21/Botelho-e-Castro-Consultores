...
export type ApiName = "brasilapi" | "receitaws" | "cnpjws" | "cache";
...
function logAttempt(attempts: AttemptLog[], a: AttemptLog) {
  attempts.push(a);
  if (!a.ok) {
    console.warn("[cnpj-lookup]", a.api, a.status, a.error ?? a.reason);
  }
}
...
export function cnpjPayloadForTool(p: CnpjNormalized): string {
  if (p.error) {
    return JSON.stringify({ erro: p.error, cnpj: p.cnpj });
  }
  return JSON.stringify({
    ...
    endereco: p.endereco,
    inscricao_estadual: p.inscricao_estadual ?? null,
    inscricao_municipal: p.inscricao_municipal ?? null,
    fonte: p._meta?.api,
  });
}
...
export async function lookupCnpjPublic(cnpjDigits: string): Promise<CnpjLookupResult> {
  const digits = cnpjDigits.replace(/\D/g, "");
  if (digits.length !== 14) {
    return {
      status: 400,
      body: { ...emptyNormalized(digits), error: "CNPJ inválido" },
    };
  }
  ...
  try {
    ...
    if (cached && cached.staleUntil > now) {
      return {
        status: 200,
        body: { ...cached.payload, _meta: { api: "cache", stale: true, attempts } },
        headers: { "X-Cache": "stale" },
      };
    }
    console.error("[cnpj-lookup] todas as fontes falharam", { cnpj: digits, attempts });
    return {
      status: 503,
      body: {
        ...emptyNormalized(digits),
        error: "Não foi possível consultar o CNPJ no momento. Tente novamente em instantes.",
        _meta: { api: "cache", stale: false, attempts },
      },
    };
  } catch (err) {
    console.error("[cnpj-lookup] exceção", {
      cnpj: digits,
    ...
    if (c && c.staleUntil > now) {
      return {
        status: 200,
        body: { ...c.payload, _meta: { api: "cache", stale: true, attempts } },
        headers: { "X-Cache": "stale" },
      };
    }
    return {
