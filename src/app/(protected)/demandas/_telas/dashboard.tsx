...
export async function DemandasDashboard({
  setor,
  searchParams,
}: {
  setor: DemandaSetor;
  ...
  const [metricas, ultimosMeses, tiposServico, usuarios, resumoPessoal] =
    await Promise.all([
      fetchDemandasParaMetricas(supabase, filtros, { agora, setor }),
      fetchDemandasUltimosMeses(supabase, filtros, { agora, setor }),
            ...
            <BarrasHorizontais
              itens={porFuncionario.map((item) => ({
                ...
                href: item.responsavelId
                  ? `${caminhos.todas}${queryStringFiltros(filtros, {
