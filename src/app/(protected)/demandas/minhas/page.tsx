...
export default async function MinhasDemandasPage({
  ...
  const [lista, tiposServico, resumo] = await Promise.all([
    fetchDemandas(supabase, filtros, { responsavelId: profile.id, agora }),
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-xs text-neutral-400 md:text-sm">
          {profile.nome} · demandas atribuídas a você.
        </p>
      </div>
      ...
      {temDestaque ? (
        <div className="flex flex-col gap-2 sm:flex-row">
          {resumo.venceHoje > 0 ? (
            <Link
              href={`/demandas/minhas${queryStringFiltros(filtros, {
                prazo: "vence_hoje",
                pagina: 1,
              })}`}
              className={clsx(
